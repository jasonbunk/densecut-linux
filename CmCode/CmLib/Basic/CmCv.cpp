...
void CmCv::FFTShift(Mat& img)
{
	...
	cv::Mat img1(img(Rect(0, 0, w, h)));
	cv::Mat img3(img(Rect(cx2, cy2, w, h)));
	cv::Mat img2(img(Rect(cx2, 0, w, h)));
	cv::Mat img4(img(Rect(0, cy2, w, h)));
	Swap(img1, img3);  // swap 1, 3
	Swap(img2, img4);  // swap 2, 4
}
...
Rect CmCv::GetMaskRange(CMat &mask1u, int ext, int thresh)
{
	...
	cv::minMaxLoc(mask1u, &minVal, &maxVal);
	if(((int)maxVal) <= thresh) {
        cout<<"GetMaskRange() will find no pixels above threshold "<<thresh<<" since max val is "<<maxVal<<endl;
	}
...
cout<<"returning Rect() at points ("<<minX<<", "<<minY<<", "<<maxX<<", "<<maxY<<")"<<endl;
...
Mat CmCv::GetBorderRegC(CMat &img3u, Mat &idx1i, vecI &idxCount)
{
	...
	Mat_<int> idx1iT;
	vecB generatedvecb;
	int regNum = CmCv::GetRegions(edg, idx1iT, idxCount, generatedvecb, true);
...
void CmCv::rubustifyBorderMask(Mat& mask1u)
{
	...
	vecB generatedvecb;
	vecI generatedveci;
	int regNum = CmCv::GetRegions(mask1u, regIdx1i, generatedveci, generatedvecb, true);
...
Mat CmCv::getGrabMask(CMat &img3u, Rect rect)
{
	...
	cv::Mat tempmask(mask1u(Rect(rect.x+1, rect.y+1, rect.width-2, rect.height-2)));
