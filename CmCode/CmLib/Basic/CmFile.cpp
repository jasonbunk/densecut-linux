...
std::string get_chars_after_last_delim(const std::string & thestring, const char thedelim) {
    size_t lastDelimPos = thestring.find_last_of(thedelim);
    if(lastDelimPos > 0 && lastDelimPos < (thestring.size()-1)) {
        return thestring.substr(lastDelimPos+1);
    }
    return "";
}
...
#else
	tinydir_dir dirr;
	int doesitexist = tinydir_open(&dirr, _path.c_str());
	tinydir_close(&dirr);
	if(doesitexist == -1) {
        const int dir_err = mkdir(_path.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
        if (-1 == dir_err) {
            cout<<"Error creating directory "<<_path<<endl;
            exit(1);
        }
    }
    return 0;
...
#else
        ifstream source(srcName, ios::binary);
        ofstream dest(dstName, ios::binary);
        dest << source.rdbuf();
        source.close();
        dest.close();
#endif
...
#else
	tinydir_dir dirr;
	tinydir_open(&dirr, folder.c_str());
	while(dirr.has_next) {
		tinydir_file file;
		tinydir_readfile(&dirr, &file);
		if(file.is_dir) {subFolders.push_back(file.name);}
		tinydir_next(&dirr);
	}
	tinydir_close(&dirr);
#endif
...
int CmFile::GetNames(CStr &nameW, vecS &names, string &dir)
{
	...
	FindClose(hFind);
#else
    std::string afterdelim(get_chars_after_last_delim(nameW,'*'));
    const int afterdelimlength = afterdelim.size();
    tinydir_dir dirr;
	tinydir_open(&dirr, dir.c_str());
	while(dirr.has_next) {
		tinydir_file file;
		tinydir_readfile(&dirr, &file);
		std::string filenamestr(file.name);
		if(file.is_dir == false && file.name[0] != '.' && filenamestr.substr(filenamestr.size()-afterdelimlength) == afterdelim) {
