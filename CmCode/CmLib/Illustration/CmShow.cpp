...
Scalar vec3btoscalar(const Vec3b & input) {
    Scalar r;
    r[0] = input[0];
    r[1] = input[1];
    r[2] = input[2];
...
const Scalar CmShow::BLACK = vec3btoscalar(Black);
const Scalar CmShow::BLUE = vec3btoscalar(Blue);
const Scalar CmShow::BROWN = vec3btoscalar(Brown);
const Scalar CmShow::GRAY = vec3btoscalar(Gray);
const Scalar CmShow::GREEN = vec3btoscalar(Green);
const Scalar CmShow::ORANGE = vec3btoscalar(Orange);
const Scalar CmShow::PINK = vec3btoscalar(Pink);
const Scalar CmShow::PURPLE = vec3btoscalar(Purple);
const Scalar CmShow::RED = vec3btoscalar(Red);
const Scalar CmShow::WHITE = vec3btoscalar(White);
const Scalar CmShow::YELLOW = vec3btoscalar(Yellow);
...
void CmShow::showTinySparseMat(CStr &title, const SparseMat &A1d)
{
	...
	struct NodeSM{
		int r, c;
		double v;
		bool operator < (const NodeSM &n) const {
			if (r < n.r)
				return true;
			else if (r > n.r)
				return false;
